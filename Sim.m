...
%% 数据记录
Para = zeros(4,sim_time);
Para_bar = zeros(4,sim_time);
...
%     time_input = time(cur_time:cur_time+data_len);
    ...
    %% 拟合
    if method == 0              % 高斯牛顿法
        [para, omega_arr] = GN(angle_input, time_input, omega);
        omega = omega_arr(end);
        % 迭代方向错误
...
%             plot((1:length(omega_arr)), ones(1, length(omega_arr)) * param(2), "green");
    ...
    elseif method == 1          % 交替迭代高斯牛顿法
        [para, omega_arr] = GN_alter(angle_input, time_input, omega);
        omega = omega_arr(end);
        % 迭代方向错误
        if (param(2)-omega)/(param(2)-omega_arr(1)) > 1
            wrong_cnt=wrong_cnt+1;
...
%             plot((1:length(omega_arr)), ones(1, length(omega_arr)) * param(2), "green");
    ...
    Angle(i) = -para(1) / para(2) * cos(para(2) * time(pre_idx) + para(3)) + (2.090 - para(1)) * time(pre_idx) + para(4);
